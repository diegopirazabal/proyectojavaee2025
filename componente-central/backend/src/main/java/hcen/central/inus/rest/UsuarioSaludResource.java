...
public class UsuarioSaludResource {
    ...
    /**
     * Desasocia un usuario de una clínica (elimina la relación)
     * DELETE /api/usuarios/{cedula}/clinica/{clinicaRut}
     */
    @DELETE
    @Path("/{cedula}/clinica/{clinicaRut}")
    public Response desasociarUsuarioDeClinica(@PathParam("cedula") String cedula,
                                               @PathParam("clinicaRut") String clinicaRut) {
        try {
            boolean deleted = usuarioService.desasociarUsuarioDeClinica(cedula, clinicaRut);
            if (deleted) {
                return Response.ok(Map.of("message", "Usuario desasociado de la clínica exitosamente")).build();
            } else {
                return Response.status(Response.Status.NOT_FOUND)
                    .entity(Map.of("error", "No se encontró la asociación usuario-clínica"))
                    .build();
            }
        } catch (IllegalArgumentException e) {
            return Response.status(Response.Status.BAD_REQUEST)
